...
import {
  Plus, Undo2, Redo2, Save, Monitor, Smartphone, Eye,
} from 'lucide-react';
...
import { SECTION_TEMPLATES, createSection, DEFAULT_SECTIONS, type EditorSection } from './types';
...
export default function StoreEditor({
  ...
  const [sections, setSections] = useState<EditorSection[]>(initialSections || DEFAULT_SECTIONS);
  const [activeId, setActiveId] = useState<string | null>(null);
  const [showAddPanel, setShowAddPanel] = useState(false);
  const [previewMode, setPreviewMode] = useState<'desktop' | 'mobile'>('desktop');
  const [showPreview, setShowPreview] = useState(true);
  const [history, setHistory] = useState<EditorSection[][]>([initialSections || DEFAULT_SECTIONS]);
  ...
  const pushHistory = useCallback((newSections: EditorSection[]) => {
    setHistory((prev) => {
  ...
  const updateSections = useCallback((newSections: EditorSection[]) => {
    ...
    <div className="h-[calc(100vh-120px)] flex gap-4">
      ...
      <div className="w-72 shrink-0 flex flex-col glass-card overflow-hidden">
        ...
        <div className="p-3 border-b border-dark-border flex items-center justify-between">
          ...
          <button onClick={handleSave} className="btn-primary text-xs px-3 py-1.5 flex items-center gap-1.5">
          ...
          {showAddPanel && (
            ...
            >
              <div className="p-3 max-h-60 overflow-y-auto space-y-1">
                <p className="text-xs text-text-muted mb-2">اختر قسماً لإضافته:</p>
                {SECTION_TEMPLATES.filter(
                  (t) => !sections.some((s) => s.type === t.type && ['hero', 'newsletter'].includes(s.type))
                ).map((template) => (
                  <button
                    key={template.type}
                    type="button"
      ...
      {showPreview && (
        <div className="flex-1 flex flex-col min-w-0">
          ...
          {/* Preview Frame */}
          <div className={`flex-1 flex items-start justify-center overflow-hidden ${previewMode === 'mobile' ? 'px-[25%]' : ''}`}>
            <div className={`h-full transition-all duration-300 ${previewMode === 'mobile' ? 'w-[375px]' : 'w-full'}`}>
              <StorePreview sections={sections} primaryColor={primaryColor} storeName={storeName} />
            </div>
          </div>
        </div>
      )}
      ...
      <div className="w-64 shrink-0 glass-card overflow-hidden flex flex-col">
        <div className="p-3 border-b border-dark-border flex items-center justify-between">
