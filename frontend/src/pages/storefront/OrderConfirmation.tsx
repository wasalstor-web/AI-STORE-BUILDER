...
import {
  ...
  ShoppingBag, Copy, Check, Search, Loader2, AlertCircle, ChevronRight,
} from "lucide-react";
...
const STATUS_CONFIG: Record<string, { label: string; icon: React.ElementType; color: string; bg: string }> = {
  pending: { label: "قيد المراجعة", icon: Clock, color: "text-amber-600", bg: "bg-amber-50" },
  confirmed: { label: "تم التأكيد", icon: CheckCircle, color: "text-blue-600", bg: "bg-blue-50" },
  processing: { label: "جاري التحضير", icon: Package, color: "text-violet-600", bg: "bg-violet-50" },
  shipped: { label: "تم الشحن", icon: Truck, color: "text-indigo-600", bg: "bg-indigo-50" },
  delivered: { label: "تم التوصيل", icon: MapPin, color: "text-emerald-600", bg: "bg-emerald-50" },
  cancelled: { label: "ملغي", icon: AlertCircle, color: "text-red-600", bg: "bg-red-50" },
};

const STATUS_STEPS = ["pending", "confirmed", "processing", "shipped", "delivered"];

export default function OrderConfirmation() {
  const { slug, orderNumber: paramOrder } = useParams<{ slug: string; orderNumber: string }>();
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8">
      ...
      <nav className="flex items-center gap-2 text-sm text-gray-400 mb-8">
        ...
        <span className="text-gray-900 font-medium">{isTrackMode ? "تتبع الطلب" : "تأكيد الطلب"}</span>
      </nav>

      {/* ── Track mode: search form ── */}
      {isTrackMode && !order && (
        <div className="max-w-md mx-auto text-center">
          <div className="w-16 h-16 mx-auto rounded-full flex items-center justify-center mb-6" style={{ backgroundColor: primaryColor + "15" }}>
            <Search className="w-8 h-8" style={{ color: primaryColor }} />
          </div>
          <h1 className="text-2xl font-bold text-gray-900 mb-2">تتبع طلبك</h1>
          <p className="text-gray-500 mb-8 text-sm">أدخل رقم الطلب والبريد الإلكتروني لمتابعة حالة طلبك</p>
          ...
          <form onSubmit={handleTrack} className="space-y-4 text-right">
            <div>
              ...
              <input
                ...
                onChange={(e) => setTrackForm((p) => ({ ...p, orderNumber: e.target.value }))}
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setTrackForm((p) => ({ ...p, email: e.target.value }))}
            ...
            >
              {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : <><Search className="w-4 h-4" /> تتبع الطلب</>}
            </button>
          </form>
        </div>
      )}
        ...
        <div className="space-y-6">
          ...
          {!isTrackMode && (
            <div className="text-center mb-8">
              ...
              <h1 className="text-2xl font-bold text-gray-900 mb-1">تم استلام طلبك بنجاح!</h1>
              <p className="text-gray-500 text-sm">شكراً لثقتك بنا. سنبدأ بتجهيز طلبك فوراً.</p>
            </div>
          )}
          ...
          <div className="bg-white border border-gray-100 rounded-2xl p-5 flex items-center justify-between">
            <div>
              ...
              <p className="text-lg font-bold text-gray-900 tracking-wide" dir="ltr">{order.order_number}</p>
            </div>
            <button
          ...
          <div className="bg-white border border-gray-100 rounded-2xl p-5">
            ...
            {order.status === "cancelled" ? (
              <div className="flex items-center gap-3 p-3 bg-red-50 rounded-xl">
                ...
                <span className="text-sm font-medium text-red-700">تم إلغاء الطلب</span>
              </div>
            ) : (
                  ...
                  return (
                    <div key={step} className="flex-1 flex flex-col items-center relative">
                      ...
                      <div
                        ...
                        style={isActive && !isCurrent ? { backgroundColor: primaryColor + "20" } : undefined}
                      >
                        <cfg.icon
                          className={`w-4 h-4 ${isCurrent ? cfg.color : isActive ? "" : "text-gray-300"}`}
                          style={isActive && !isCurrent ? { color: primaryColor } : undefined}
                        />
                      </div>
                      <p className={`text-[10px] mt-1.5 text-center ${isCurrent ? "font-bold text-gray-900" : "text-gray-400"}`}>
                        {cfg.label}
                      </p>
                    </div>
                  );
          ...
          {/* Order Items */}
          <div className="bg-white border border-gray-100 rounded-2xl p-5">
            <h3 className="text-sm font-bold text-gray-900 mb-4">تفاصيل الطلب</h3>
            ...
            <div className="border-t border-gray-100 mt-4 pt-3 space-y-1.5 text-sm">
              ...
              <div className="flex justify-between text-gray-500">
                <span>الشحن</span>
                <span className="text-emerald-600 font-medium">{order.shipping_cost > 0 ? formatPrice(order.shipping_cost) : "مجاني"}</span>
              </div>
