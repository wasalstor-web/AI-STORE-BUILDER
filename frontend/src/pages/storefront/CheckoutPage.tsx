...
import {
  CreditCard, Truck, Lock, Loader2, ChevronRight, ShoppingBag,
...
const PAYMENT_METHODS = [
  ...
  { id: "visa", label: "فيزا / ماستركارد", icon: CreditCard, desc: "بطاقات الائتمان" },
  { id: "stc_pay", label: "STC Pay", icon: Smartphone, desc: "الدفع عبر STC Pay" },
];

const CITIES = [
  "الرياض", "جدة", "مكة المكرمة", "المدينة المنورة", "الدمام",
  "الخبر", "الظهران", "تبوك", "بريدة", "الطائف", "أبها", "خميس مشيط",
  "حائل", "نجران", "جازان", "الأحساء", "ينبع",
];
...
export default function CheckoutPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      clearCart();
      navigate(`/store/${slug}/order/${orderNumber}?email=${encodeURIComponent(form.email)}`);
    } catch (err: unknown) {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <nav className="flex items-center gap-2 text-sm text-gray-400 mb-8">
        <Link to={`/store/${slug}/cart`} className="hover:text-gray-600 transition-colors">السلة</Link>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {PAYMENT_METHODS.map((method) => (
                  <button
                    ...
                    style={paymentMethod === method.id ? { borderColor: primaryColor, backgroundColor: primaryColor + "08" } : undefined}
                  >
                    <method.icon
                      className="w-5 h-5 flex-shrink-0"
                      style={{ color: paymentMethod === method.id ? primaryColor : "#9ca3af" }}
                    />
                    <div>
